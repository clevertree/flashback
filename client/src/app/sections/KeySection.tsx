...
export default function KeySection({defaultConfigPath, onKeyVerified}: KeySectionProps) {
    // State for form
    ...
    useEffect(() => {
        let cancelled = false;
        const check = () => {
            const cryptoIsAvailable = isCryptoAvailable();
            if (!cancelled) setCryptoAvailable(cryptoIsAvailable);
        ...
        if (!check()) {
            ...
            return () => {
                cancelled = true;
                window.removeEventListener('flashbackCryptoReady', onReady as any);
                window.clearInterval(id);
            };
        }
        return () => {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                const res = await flashbackCrypto()!.checkKeyExists();
                if (cancelled) return;
                if (res?.status === 'valid') {
                    setSuccess(true);
                    setPrivateKeyPath(res.privateKeyPath);
    ...
    const handleGenerate = useCallback(async () => {
        ...
        try {
            const existsRes = await flashbackCrypto()!.checkKeyExists().catch(() => ({status: 'missing' as const}));
            if (existsRes?.status === 'valid') {
        ...
        try {
            ...
            const res = await flashbackCrypto()!.generateUserKeysAndCert({
                email,
                ...
                <div className="grid sm:grid-cols-2 gap-3 max-w-3xl">
                    ...
                    <div className="sm:col-span-2 flex flex-col gap-2 mt-1">
                        <div className="flex gap-2">
                            ...
                            <input ref={fileInputRef} type="file" accept=".key,.pem" className="hidden"
                        ...
                        {(!cryptoAvailable) && (
                            <div className="text-red-600 text-sm">Crypto features are unavailable; generation is
                                disabled.</div>
                        )}
                        {error && (
            ...
            {success && (
                <div className="max-w-3xl border rounded p-3 bg-gray-50">
                    <div className="text-sm text-gray-700 space-y-1">
                        {privateKeyPath && (
                            <div><span className="text-gray-500">Private Key:</span> <code>{privateKeyPath}</code>
                            </div>)}
                        {certPath && (
                            <div><span className="text-gray-500">Certificate:</span> <code>{certPath}</code></div>)}
                    </div>
                    <div className="flex gap-2 mt-3">
                        <button className="px-3 py-2 border rounded" onClick={resetToEdit}>Select or Generate a New
                            Key
                        </button>
                    </div>
                </div>
